function [distHistory] = tsp_ga_ugv(INSTANCE, popSize, tournaments, mutOper,mutRate,crossOper, eliteP)

 % This function solves the TSP for the UGV in the TERRA algorithm.
 
 % INPUTS
 % INSTANCE: location of the instance with the target point distribution
 ...
 % mutRate: mutation rate from 0.0 to 1.0
 % crossOper: crossover operator (1 = OX; 2 = CX; 3 = CBX)
 % eliteP: elitism selection from 0 to 100%.
 
 % OUTPUT
 % distHistory: matrix of 3 rows (min, mean, max) where every column is the
 % distance results on every generation.
